...
def get_args():
    ...
    parser.add_argument(
        "--server_address", type=str, default="http://localhost:8484", help="Address of the TGI server"
    )
    parser.add_argument(
        "--model_id", type=str, default="/mnt/weka/data/pytorch/llama2/Llama-2-7b-chat-hf", help="Model id used in TGI server"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--total_sample_count", type=int, default=2, help="Total number of samples to generate"
    )
    parser.add_argument(
        "--max_concurrent_requests", type=int, default=128, help="Max number of concurrent requests"
    )
...
def read_dataset(
    ...
    with open('DIBT_dataset_len256.json', 'r', encoding='utf-8') as f:
            input_sentences = json.load(f)
    return input_sentences[:total_sample_count]
